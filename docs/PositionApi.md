...
with mfexapiclient.ApiClient(configuration) as api_client:
    ...
    api_instance = mfexapiclient.PositionApi(api_client)
    office_id = 'office_id_example' # str | 事業所id
    unknown_base_type = mfexapiclient.UNKNOWN_BASE_TYPE() # UNKNOWN_BASE_TYPE | 役職のパラメータ
